...
const Icons = {
  ...
  Upload: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>,
  Warning: (props) => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
};
...
export default function AdminRegistrationGuidelines({ apiFetch, toast }) {
  ...
  /* Import from the "הנחיות כלליות לרישום" DOCX.
     The server parses and filters to this semester; nothing is written until the
     admin reviews the merged fields and presses save. */
  const importDocx = async (file) => {
    ...
    try {
      ...
      setDirty(true);
      setImportResult({ meta: data.meta || {}, warnings: data.warnings || [] });
      toast("ok", "✅ הקובץ נותח. יש לבדוק את השדות ולשמור");
    } catch (e) {
      toast("error", `⚠️ ${e.message}`);
    } finally {
      setUploading(false);
      ...
      {view === "rules" && (
        <div className="space-y-6">
          <Card className="border-t-4 border-t-emerald-500">
            <SectionHeader title="ייבוא מקובץ הנחיות רישום (DOCX)" icon={Icons.Upload} />
            <div className="space-y-3">
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-5">
                העלאת הקובץ "הנחיות כלליות לרישום" תמלא אוטומטית את הכללים, הקישורים, מגבלות הנ״ז
                ואנשי הקשר הרלוונטיים לסמסטר {semester}. שום דבר לא נשמר עד ללחיצה על "שמירה".
              </p>
              <div className="flex flex-wrap items-center gap-3">
                <input
                  ...
                  className="text-xs file:me-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 dark:file:bg-emerald-900/30 dark:file:text-emerald-300"
                />
                {uploading && (
                  <span className="text-xs text-slate-500 animate-pulse">מנתח את הקובץ...</span>
                )}
              </div>

              {importResult && (
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3 text-xs dark:bg-slate-800/40 dark:border-slate-700">
                  <div className="font-semibold text-slate-700 dark:text-slate-200">
                    {importResult.meta.title || "נותח בהצלחה"}
                    {importResult.meta.termText ? ` · ${importResult.meta.termText}` : ""}
                  </div>
                  {importResult.warnings.length > 0 && (
                    <ul className="mt-2 space-y-1">
                      {importResult.warnings.map((w, i) => (
                        <li key={i} className="flex gap-1.5 items-start text-amber-700 dark:text-amber-400">
                  ...
                  {!importResult.warnings.length && (
                    <div className="mt-1 text-emerald-600 dark:text-emerald-400">לא נמצאו חוסרים בקובץ</div>
                  )}
                </div>
              )}
            </div>
